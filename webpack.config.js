...
module.exports = env => {
  ...
  return {
          ...
          use: { // Use the following loader and options
            ...
            options: {
              presets: [
                ...
                // The react preset includes several plugins that are required to write a React app. For example it transforms JSX into React.createElement() calls
                '@babel/preset-react',
              ],
            },
          }
    ...
    plugins: [
      // This plugin generates an HTML5 file that imports all our Webpack bundles using <script> tags. The file will be placed in `output.path`. See: https://github.com/jantimon/html-webpack-plugin
      new HtmlWebpackPlugin({
        template: path.join(PATH_SOURCE, './index.html'),
      }),

      // This plugin will delete all files inside `output.path` but the directory itself will be kept. See: https://github.com/johnagan/clean-webpack-plugin
      new CleanWebpackPlugin(),
    ],

    // Configuration options for Webpack DevServer, an Express web server that aids with development. It provides live reloading out of the box and can be configured to do a lot more.
    devServer: {
      ...
      // When using the HTML5 History API (you'll probably do this with React later), index.html should be served in place of 404 responses
